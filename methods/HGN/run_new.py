...
def run_model_DBLP(args):
    ...
    for _ in range(args.repeat):
        ...
        np.savez('save/loss_record',train = train_loss_list,val = val_loss_list)
        ...
        with torch.no_grad():
            ...
            dl.gen_file_for_evaluate(test_idx=test_idx, label=pred, file_name=f"output/save/{args.dataset}_{args.run}.txt")
            pred = onehot[pred]
            np.savez('save/label',predict = pred,val = dl.labels_test['data'][dl.labels_test['mask']])
            # print(dl.evaluate(pred))
            
            # 读取预测结果和真实标签
            predict_onehot = pred
            groundtruth_onehot = dl.labels_test['data'][dl.labels_test['mask']]
